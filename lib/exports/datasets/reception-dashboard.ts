...
export class ReceptionDashboardDataset implements Dataset {
  ...
  async queryPage(ctx: ExportContext, f: z.infer<typeof Filter>) {
    ...
    const snapshot = await query<{ waiting: string; in_service: string; done: string }>(
      `SELECT COUNT(*) FILTER (WHERE status='waiting')::int AS waiting,
              COUNT(*) FILTER (WHERE status='in_service')::int AS in_service,
              COUNT(*) FILTER (WHERE status='done')::int AS done
       FROM queues q WHERE ($1::text IS NULL OR department = $1)`,
      [dept]
    )
    ...
    rows.push({ section: 'KPIs', metric: 'Arrivals', value: String(arrivals.rows?.[0]?.count ?? 0) })
    rows.push({ section: 'KPIs', metric: 'Serviced', value: String(serviced.rows?.[0]?.count ?? 0) })
    rows.push({ section: 'KPIs', metric: 'Completed', value: String(completed.rows?.[0]?.count ?? 0) })

    const snap = snapshot.rows?.[0] as any || { waiting: 0, in_service: 0, done: 0 }
    rows.push({ section: 'Queue Snapshot', metric: 'Waiting', value: String(snap.waiting ?? 0) })
    rows.push({ section: 'Queue Snapshot', metric: 'In Service', value: String(snap.in_service ?? 0) })
    rows.push({ section: 'Queue Snapshot', metric: 'Done', value: String(snap.done ?? 0) })
    ...
    // Payments by Method (for dashboard compact chart and analysis)
    ...
    // Per-department breakdown (queue snapshot + KPIs)
