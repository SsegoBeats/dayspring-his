...
export async function POST(req: Request) {
  try {
    ...
    const systemQuantity = Number(medRows[0].stock_quantity) || 0
    ...
    const { rows } = await queryWithSession(
      ...
      `INSERT INTO stock_taking (
       ...
       RETURNING id, medication_id, recorded_quantity, system_quantity, variance, notes, taken_at, status`,
      [medicationId, recordedQuantity, systemQuantity, variance, notes, auth.userId],
    )

    return NextResponse.json({ stockTaking: rows[0] }, { status: 201 })
  } catch (err: any) {
    console.error("Error recording stock taking:", err)
...
export async function GET(req: Request) {
  try {
    ...
    let where = "WHERE 1=1"
    if (medicationId) {
      where += " AND st.medication_id = $" + (params.length + 1)
    ...
    const { rows } = await queryWithSession(
      ...
      `
        SELECT st.id,
          ...
          JOIN medications m ON m.id = st.medication_id
    ...
    console.error("Error fetching stock takings:", err)
...
export async function PATCH(req: Request) {
  try {
    ...
    const { rows: stRows } = await queryWithSession(
      { role: auth.role, userId: auth.userId },
      `SELECT medication_id, variance FROM stock_taking WHERE id = $1`,
      [id],
    )
    ...
    const { medication_id, variance } = stRows[0]

    // Update status
    await queryWithSession(
      { role: auth.role, userId: auth.userId },
      `UPDATE stock_taking SET status = $1 WHERE id = $2`,
      [status, id],
    )
    ...
    console.error("Error updating stock taking:", err)
