...
export async function GET(req: Request) {
  try {
    ...
    const buf = await toXLSX(
      ...
      {
        meta: { title: 'Laboratory Results', exportedBy: String(auth.userId), timestamp: new Date().toISOString() },
        columns: ['Patient','P.ID','Test','Parameter','Value','RefRange','Flag','Accession','Ordered','Completed'],
        headerTitle,
        headerSubtitle,
        logoDataUrl,
      }
    )
    return new NextResponse(Buffer.from(buf), { status: 200, headers: { 'Content-Type': 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet', 'Content-Disposition': `attachment; filename=labs-${new Date().toISOString().slice(0,10)}.xlsx` } })
  } catch (e:any) {
    console.error('XLSX export failed:', e?.message || e)
    return NextResponse.json({ error: 'Failed to render XLSX' }, { status: 500 })
  }
}
