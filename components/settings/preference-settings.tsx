      ...
      try {
        ...
        if (res.ok) {
          ...
          if (data.preferences) {
            ...
            // Apply theme on load and persist to localStorage so pre-hydration can pick it up
            if (!previewedRef.current) {
              setTheme(fetchedPreferences.theme)
              try { window.localStorage?.setItem('theme', fetchedPreferences.theme) } catch {}
            }
          }
        }
      } catch (error) {
  ...
  const savePreferences = async () => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        try { window.localStorage?.setItem('theme', preferences.theme) } catch {}
